    ...
    ) 
{
  ...
  if(size > COAP_MAX_PDU_SIZE)
    m2m_assert(size <= COAP_MAX_PDU_SIZE, NULL);
  /* Size must be large enough to fit the header. */
  if (size < sizeof(coap_hdr_t) || size > COAP_MAX_PDU_SIZE)
    return NULL;

  pdu = (coap_pdu_t *)mmalloc(sizeof(coap_pdu_t) + size);
...
coap_pdu_t *WD_SYSTEM coap_new_pdu() 
{
  ...
  pdu = coap_pdu_init(0, 0, \
                      m2m_ntohs(COAP_INVALID_TID), M2M_PROTO_PKT_MAXSIZE);
  return pdu;
}
    ...
    ) 
{
  ...
  if (!optsize) 
  {
    m2m_debug_level(M2M_LOG_WARN, "pdu:: cannot add option\n");
    /* error */
    return NULL;
  } 
  else 
...
{
  m2m_assert(pdu, FALSE);
  m2m_assert(len, FALSE);
  m2m_assert(data, FALSE);
    ...
    ) 
{
  ...
  {
    m2m_debug_level(M2M_LOG_ERROR, "pdu:: discarded invalid PDU");
  }
  ...
  {
    m2m_debug_level(M2M_LOG_ERROR, "pdu:: coap_pdu_parse: invalid Token");
    goto discard;
  }
  ...
  while (length && *opt != COAP_PAYLOAD_START) 
  {
    ...
    {
      m2m_debug_level(M2M_LOG_ERROR, "pdu:: coap_pdu_parse: drop");
      goto discard;
    }
  }
  ...
  if (length) 
  {
    m2m_assert(*opt == COAP_PAYLOAD_START, 0);
    opt++; length--;
...
int WD_SYSTEM coap_show_pdu(const coap_pdu_t *pdu) 
{
  ...
  fprintf(COAP_DEBUG_FD, "v:%d t:%d tkl:%d c:%d id:%u",
      ...
      pdu->hdr->code, m2m_ntohs(pdu->hdr->id));
  ...
  if (pdu->data) 
  {
    m2m_assert(pdu->data < (unsigned char *)pdu->hdr + pdu->length, 0);
