...
int data_enc(const char *src, char *dst, int len, int keylen, u8 *p_key)
{
    ...
    while(len >= 16)
    {
        ...
        AES128_ECB_encrypt((uint8_t*)(src + count), (uint8_t*)p_key, (uint8_t*)(dst + count));
        count += 16;
        len -= 16;
    }
    if(len > 0)
    {
        ...
        AES128_ECB_encrypt((uint8_t*)tmp_input,(uint8_t*) p_key, (uint8_t*)(dst + count));
        count += 16;
    }
    
    //m2m_bytes_dump("after encode data :", dst, count);
    return count;
}
...
//  aes_context ctx;
    ...
    while(len >= 16)
    {
        ...
        AES128_ECB_decrypt((uint8_t*)(src + count), (uint8_t*)p_key, (uint8_t*)(dst + count));
        count += 16;
        len -= 16;
    }
