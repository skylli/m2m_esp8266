...
#define ADVANCE_OPT(o,e,step) if ((e) < step) {                          \
   m2m_debug_level(M2M_LOG_ERROR, "cannot advance opt past end\n"); \
    return 0;                                                            \
  } else {                                                               \
    (e) -= step;                                                         \
  ...
  switch(result->delta) 
  {
      ...
      case 14:
        ...
        if (result->delta < 269)
        {
          m2m_debug_level(M2M_LOG_ERROR, "delta too large\n");
          return 0;
        }
        /* fall through */
      case 13:
  ...
  {
      case 15:
        m2m_debug_level(M2M_LOG_ERROR, "found reserved option length 15\n");
        return 0;
      case 14:
        /* Handle two-byte value: First, the MSB + 269 is stored as delta value.
         * After that, the option pointer is advanced to the LSB which is handled
  ...
  if (length < result->length) {
    m2m_debug_level(M2M_LOG_ERROR, "invalid option length\n");
    return 0;
  }
    ...
    )
{
  ...
  m2m_assert((sizeof(coap_hdr_t) + pdu->hdr->token_length) <= pdu->length, \
                  NULL);
...
coap_opt_t *WD_SYSTEM coap_option_next(coap_opt_iterator_t *oi) 
{
  ...
  while (1) 
  {
    ...
    if (optsize) 
    {
      m2m_assert(optsize <= oi->length, NULL);
...
unsigned short WD_SYSTEM coap_opt_length(const coap_opt_t *opt) 
{
  ...
  {
      case 0xf0:
        m2m_debug_level(M2M_LOG_WARN, "illegal option delta\n");
        return 0;
      case 0xe0:
        ++opt;
  ...
  {
      case 0x0f:
        m2m_debug_level(M2M_LOG_ERROR, "illegal option length\n");
        return 0;
      case 0x0e:
        length = (*opt++ << 8) + 269;
        /* fall through */
...
unsigned char *WD_SYSTEM coap_opt_value(coap_opt_t *opt) 
{
  ...
  {
      case 0xf0:
        m2m_debug_level(M2M_LOG_ERROR, "illegal option delta\n");
        return 0;
      case 0xe0:
        ++ofs;
        /* fall through */
